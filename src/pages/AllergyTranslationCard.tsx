...
const AllergyTranslationCard = () => {
  return (
    <>
      <Helmet>
        {/* react-helmet-async requires every Helmet child's content to be a
            single string — mixing text with a raw {number} here produces a
            [string, number, string] children array, which throws
            ("Helmet expects a string as a child of <title>") and, with no
            ErrorBoundary at the time, took down the entire site. Wrapping
            the whole thing in one template literal makes it a single string,
            same pattern already used correctly below for every meta
            content="" attribute. */}
