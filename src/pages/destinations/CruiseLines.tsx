...
const CruiseLines = () => {
  ...
  return <div className="min-h-screen bg-background pb-12">
      <Helmet>
        ...
        <meta name="description" content="Explore the best cruise lines for travelers with food allergies. Real reviews, expert rankings, and top safety practices for celiac, nut, dairy, and gluten sensitivities." />
        <meta name="keywords" content="cruise lines food allergies, allergy-friendly cruises, gluten-free cruise, dairy-free cruise meals, cruises for dietary restrictions, celiac cruise, allergy-safe travel" />
      ...
      <div className="relative h-[40vh] sm:h-[45vh] md:h-[50vh] overflow-hidden">
        <img src="https://images.unsplash.com/photo-1548574505-5e239809ee19?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80" alt="Luxury cruise ship sailing on blue ocean - allergy-friendly cruising" className="absolute inset-0 w-full h-full object-cover" />
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl -mt-10 relative z-10">
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          <section className="mb-12">
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200">
              <Table>
                ...
                <TableBody>
                  {cruiseLines.map(cruise => <TableRow key={cruise.name}>
                      <TableCell className="font-medium">{cruise.name}</TableCell>
                      <TableCell>
                        <div className="flex">
                          {Array.from({
                        length: 5
                      }).map((_, i) => <Star key={i} className={`h-4 w-4 ${i < cruise.rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`} />)}
                        </div>
                      </TableCell>
                      <TableCell>
                        {cruise.familyFriendly ? <Check className="h-5 w-5 text-green-600" /> : <AlertTriangle className="h-5 w-5 text-yellow-600" />}
                      </TableCell>
                      <TableCell>
                        {cruise.buffetLabels === true ? <Check className="h-5 w-5 text-green-600" /> : cruise.buffetLabels === 'partial' ? <AlertTriangle className="h-5 w-5 text-yellow-600" /> : <AlertTriangle className="h-5 w-5 text-red-600" />}
                      </TableCell>
                      <TableCell>
                        {cruise.chefConsult ? <Check className="h-5 w-5 text-green-600" /> : <AlertTriangle className="h-5 w-5 text-yellow-600" />}
                      </TableCell>
          ...
          <section className="space-y-12">
            ...
            {cruiseLines.map((cruise, index) => <div key={cruise.name} className="mb-10">
                ...
                <div className="bg-gray-50 p-5 rounded-lg">
                  ...
                  <ul className="list-disc list-inside space-y-1 mb-4 ml-2">
                    {cruise.features?.map((feature, i) => <li key={i} className="text-muted-foreground">{feature}</li>)}
                  </ul>
                  
                  {cruise.quote && <blockquote className="border-l-4 border-primary/30 pl-4 italic text-muted-foreground">
