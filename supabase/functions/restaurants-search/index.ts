...
// just the top result, not the whole list — see hotel-search's copy of
// this comment for the cost math (~400 distinct new places/month
// site-wide vs. one lookup per unique destination search).
...
serve(async (req) => {
  ...
  try {
    ...
    if (supabaseUrl) {
      await enrichTopResultWithTripadvisor(results, destination, 'restaurant', supabaseUrl);
    }
